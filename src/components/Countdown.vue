...
import {countDown} from "~/api/app/countDown.js";
...
const displayText = ref("00:00:00");
...
function flashText() {
  ...
  seconds = seconds - 1;
  displayText.value = countdownTimes(seconds);
}
...
function connection() {
  ...
  // 建立连接对象
  const ws = new WebSocket("ws://127.0.0.1:35722/ws"); // 连接服务端提供的通信接口，连接以后才可以订阅广播消息和个人消息
  // 获取STOMP子协议的客户端对象
...
<template>
  <div className="hello">
    <div id="inspectionCountdown" className="font-time">{{displayText}}</div>
  </div>
</template>
